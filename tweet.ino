...
// timevalue is needed to keep track on unixtimestamp
// also used in oauth_nonce and oauth_timestamp
int timevalue = 0;
  ...
  static void process_(std::vector<std::string> *args, http_method_t http_method, const Keys &keys)
  {
...

    {
      ...
      if (!is_key_contains(*args, oauth_timestamp)) {
        oauth_timestamp += '=';
        oauth_timestamp += to_s(timevalue);
      ...
      if (opt.upload_path.empty()) {
        ...
        if (client.connect(host.c_str(), port)) {
          ...
          *reply = std::string(s.c_str(), s.length());
          // this has the post content we are sending
          // Serial.println(opt.post_begin);
          // this has the response. Would need some json parsing
...
void loop(void){
  ...
  delay(500);
  if(1 == 0){
    // call here, use the _same_ timevalue within functions
    timevalue = ntpClient.getUnixTime();
    TwitterClient tc(consumer_key, consumer_sec, accesstoken, accesstoken_sec);
    String s = "Hello World";
    std::string text = std::string(s.c_str(), s.length());
    // returns boolean, but no actual returning of reason if fails
    tc.tweet(text);
  }
